...
/**
 * The onboarding screen behind both onboarding routes, with the flow it runs
 ...
 *
 * `agentId` names which of the tenant's onboarding agents to hand over on the
 * closing step — the id in `/platform/<tenant>/onboarding/<agent-id>`, the link
 * an admin copies out of the admin panel. Without one the member flow uses the
 * first agent the tenant configured.
 *
 * Distinct from `/platform/[tenant]/start`, the built-in learner start screen.
 */
export function OnboardingFlowPage({ agentId }: { agentId?: string }) {
  ...
  // and forced to the member flow by an agent link.
  ...
  return (
    ...
    <div className="flex min-h-0 flex-1 flex-col pt-4" data-testid="onboarding-page">
      <OnboardingWizard
        // Remounts on the switch — and on a move between agent links — so no
        // run inherits another's step or answers.
        key={`${userFlow ? 'user' : 'admin'}:${agentId ?? ''}`}
        tenant={tenant}
        username={getUserName()}
        brandName={brandName}
        isAdminOnboarding={!userFlow}
        // Which configured agent closes the member flow. An id naming none
        // shows the wizard's "nothing set up" screen rather than another
        // audience's agent.
        agentId={agentId}
