...
const mockSearchParams = new URLSearchParams('tenant=test-tenant');
...
describe('ErrorPage', () => {
  ...
  describe('error code rendering', () => {
    ...
    it('renders 409 title and description', () => {
      vi.mocked(useParams).mockReturnValue({ code: '409' });
      render(<ErrorPage />);
      expect(screen.getByText('Unauthorized Organization')).toBeInTheDocument();
      expect(
        screen.getByText(
  ...
  describe('tenant query param', () => {
    it('resolves the tenant support email from the ?tenant= query param', () => {
      vi.mocked(useParams).mockReturnValue({ code: '409' });
      render(<ErrorPage />);
      expect(vi.mocked(useTenantMetadata)).toHaveBeenCalledWith({ org: 'test-tenant' });
    });
  });
