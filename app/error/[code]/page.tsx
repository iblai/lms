...
const ERROR_MAP: Record<string, { title: string; description: string; icon: React.ReactNode }> = {
  ...
  'unauthorized-tenant': {
    title: 'Unauthorized Resource',
    description: "The resource you're trying to access belongs to a different platform and cannot be accessed here.",
    icon: (
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5} className="w-16 h-16">
        <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z" />
      </svg>
  ...
  return (
    <div className="flex-1 flex items-center justify-center px-6">
      <div className="max-w-lg w-full text-center">
        ...
        {/* Error code badge */}
        <span className="inline-block mb-4 rounded-full bg-amber-100 px-4 py-1 text-xs font-semibold uppercase tracking-widest text-amber-600">
          {code}
        </span>

        <h1 className="mb-3 text-2xl font-bold text-gray-800">{title}</h1>
        <p className="mb-8 text-sm text-gray-500 leading-relaxed">{description}</p>

        {/* Actions */}
        <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
          <Link
            href="/"
            className="w-full sm:w-auto px-6 py-2.5 rounded-md bg-amber-500 text-white text-sm font-medium hover:bg-amber-600 transition-colors"
          >
            Back to Home
          </Link>
          <a
            href={`mailto:${supportEmail}`}
            className="w-full sm:w-auto px-6 py-2.5 rounded-md border border-amber-300 text-amber-600 text-sm font-medium hover:bg-amber-50 transition-colors"
          >
            Contact Support
          </a>
        </div>
