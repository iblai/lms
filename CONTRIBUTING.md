...
2. Clone your fork:
   ```bash
   git clone https://github.com/<your-username>/skills-ai.git
   cd skills-ai
   ```
...
- **API services** are defined in `services/` using RTK Query
- **Reusable hooks** go in `hooks/`, organized by domain (`courses/`, `profile/`, `skills/`)
...
- Use RTK Query hooks (`useGetXQuery`, `useLazyGetXQuery`, `useXMutation`) for API calls
- Local UI state stays in React state (`useState`, `useReducer`)
- Shared UI state goes through Redux slices in `features/`

### File Naming

- Components: `PascalCase.tsx` or `kebab-case.tsx` (follow existing patterns in each directory)
- Hooks: `use-kebab-case.ts` (e.g., `use-course-detail.ts`)
- Utilities: `kebab-case.ts` (e.g., `helpers.ts`)
- Tests: `*.test.ts` or `*.test.tsx` in a `__tests__/` directory
- Types: `kebab-case.ts` (e.g., `courses.ts`, `skills.ts`)
...
- Use the `@iblai/iblai-js` SDK for all data layer, auth, and shared component imports:
  ```typescript
  import { useLazyGetOverTimeActivityQuery, StorageService } from '@iblai/iblai-js/data-layer';
  import { AuthProvider, TenantProvider, useTenantMetadata } from '@iblai/iblai-js/web-utils';
  import { Loader, EducationDialog, ExperienceDialog } from '@iblai/iblai-js/web-containers';
  import { SsoLogin, UserProfileModal } from '@iblai/iblai-js/web-containers/next';
  ```
...
If you have questions about contributing, open a [Discussion](https://github.com/iblai/skills-ai/discussions) or reach out to the maintainers.
