...
The pre-commit hook runs `pnpm dlx lint-staged --verbose` which formats staged files automatically. If Claude's edits are already formatted correctly, the commit goes through cleanly with no diff noise. If they aren't, the hook rewrites the files and the commit includes both code changes and formatting noise — harder to review.
