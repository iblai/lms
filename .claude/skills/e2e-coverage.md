...
- A completely new feature/page was added
...
2. **CI workflow**: The coverage check job runs the same script with `--all --no-regress` and blocks merge on failure.
...
1. **Write/update the spec file** in `e2e/journeys/` following the existing patterns:
   ...
   - Use `SKILL_HOST` from `process.env.SKILLS_HOST`
