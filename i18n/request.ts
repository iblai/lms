 ...
 *
 * Cookie-based rather than route-based on purpose: it keeps every existing
 * route untouched — no `[locale]` segment, no middleware rewrite — and matches
 * how `os` resolves the same preference.
 */
export default getRequestConfig(async () => {
  const cookieStore = await cookies();
  const locale = resolveLocale(
    cookieStore.get(OPENEDX_LOCALE_COOKIE)?.value ?? cookieStore.get(LOCALE_COOKIE)?.value,
  );
