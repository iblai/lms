...
test.describe('Journey 17: Analytics Users', () => {
  ...
  test('CP-1: users analytics page loads with metrics', async ({ page }) => {
    ...
    const usersContent = page.locator(
      '[class*="user"], [data-testid*="user"], [class*="metric"], [class*="chart"]'
    ).first()
  ...
  test('CP-2: registered users sub-page loads', async ({ page }) => {
    ...
    const registeredTab = page.getByRole('tab', { name: /registered/i })
  ...
  test('CP-3: active users sub-page loads', async ({ page }) => {
    ...
    const content = page.locator(
      '[class*="metric"], [class*="chart"], [class*="active"]'
    ).first()
      .or(page.getByRole('main'));
  ...
  test('CP-4: at-risk users sub-page loads', async ({ page }) => {
    ...
    const atRiskTab = page.getByRole('tab', { name: /at.risk|at risk/i })
    ...
    // Verify content area
    const content = page.locator(
      '[class*="metric"], [class*="chart"], [class*="risk"]'
    ).first()
      .or(page.getByRole('main'));
    await expect(content).toBeVisible({ timeout: 15_000 });
  });
  ...
  test('CP-5: time filter updates data', async ({ page }) => {
    ...
    const pageContentBefore = await page.getByRole('main').textContent().catch(() => '');
