...
test.describe('Journey 01: Authentication', () => {
  ...
  test('Checkpoint 2: User can authenticate via SSO flow', async ({
    page,
  }) => {
    if (!AUTH_HOST) {
      test.skip();
    ...
    expect(
      postLoginUrl.includes('/home') || postLoginUrl.includes('/start')
    ).toBeTruthy();
  });

  test('Checkpoint 3: After SSO completion user lands on /home or /start', async ({
    page,
  }) => {
    ...
    expect(
      landingUrl.includes('/home') || landingUrl.includes('/start')
    ).toBeTruthy();
  });

  test('Checkpoint 4: Unauthenticated access to /home is redirected', async ({
    page,
  }) => {
    if (!AUTH_HOST) {
      test.skip();
