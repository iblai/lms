...
test.describe('Journey 19: Analytics Topics/Transcripts/Financial', () => {
  ...
  test('CP-1: topics analytics loads with charts', async ({ page }) => {
    ...
    const content = page.locator(
      '[class*="chart"], [class*="topic"], [data-testid*="topic"], canvas, svg'
    ).first()
  ...
  test('CP-2: topics distribution data is displayed', async ({ page }) => {
    ...
    const distributionContent = page.locator(
    ...
    const hasDistribution = await distributionContent.isVisible({ timeout: 15_000 }).catch(() => false);
  ...
  test('CP-3: transcripts analytics loads with metrics', async ({ page }) => {
    const transcriptsTab = page.getByRole('tab', { name: /transcripts/i })
      .or(page.getByRole('link', { name: /transcripts/i }));
    const hasTranscriptsTab = await transcriptsTab.isVisible({ timeout: 15_000 }).catch(() => false);
  ...
  test('CP-4: transcripts costs and ratings are displayed', async ({ page }) => {
    ...
    const hasMetrics = await metricsCards.first().isVisible({ timeout: 5_000 }).catch(() => false);
  ...
  test('CP-5: financial analytics loads with cost cards', async ({ page }) => {
    ...
    const hasCostCards = await costCards.first().isVisible({ timeout: 10_000 }).catch(() => false);
  ...
  test('CP-6: financial cost breakdowns are displayed', async ({ page }) => {
    ...
    const breakdownContent = page.locator(
      '[class*="breakdown"], [class*="chart"], canvas, svg, table, [class*="cost-detail"]'
    ).first();
