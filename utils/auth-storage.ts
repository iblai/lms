...
export function isPerTabAuthEnabled(): boolean {
  // Runtime __ENV__ (entrypoint.sh) wins, then the build-time inlined value.
 ...
 */
export function clearPerTabSession(opts?: { clearSeedIfTenant?: string }): void {
  if (typeof window === 'undefined') return;
  for (const key of PER_TAB_AUTH_KEYS) {
    window.sessionStorage.removeItem(key);
  }
  const clearSeedIfTenant = opts?.clearSeedIfTenant;
  if (clearSeedIfTenant && window.localStorage.getItem('tenant') === clearSeedIfTenant) {
    for (const key of PER_TAB_AUTH_KEYS) {
      window.localStorage.removeItem(key);
    }
  }
